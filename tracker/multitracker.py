...
class JDETracker(object):
    def __init__(self, opt, frame_rate=30):
        ...
        if isinstance(opt.weights, list):
            weights_path = opt.weights[0]  # 使用列表中的第一个权重文件
        else:
            weights_path = opt.weights
        
        # 添加 numpy.core.multiarray._reconstruct 到安全全局对象列表
        add_safe_globals([np.core.multiarray._reconstruct])
        
        try:
            ckpt = load(weights_path, map_location='cpu', weights_only=True)
        except Exception as e:
            print(f"Error loading weights with weights_only=True: {e}")
            print("Attempting to load weights without weights_only...")
            ckpt = load(weights_path, map_location='cpu')
    ...
    def update(self, im_blob, img0, path):
        ...
        if len(pred) > 0:
            ...
            detections = [STrack(STrack.tlbr_to_tlwh(tlbrs[:4].cpu().numpy()), tlbrs[4].item(), f.cpu().numpy(), 30) for
