...
from models.common import Conv, Bottleneck, SPP, DWConv, Focus, BottleneckCSP, Concat, C3, SPPF
...
class Detect(nn.Module):
    def __init__(self, nc=80, anchors=(), ch=(), emb_dim=256):  # detection layer
        ...
        self.log_scale = torch.log(torch.tensor(8.0 / 640**2))
...
class Model(nn.Module):
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            b = mi.bias[:t_dim-m.emb_dim].detach().view(m.na, -1)  # conv.bias(255) to (3,85)
            b[:, 4] = b[:, 4] + m.log_scale.to(b.device, b.dtype) * s**2  # obj (8 objects per 640 image)
            b[:, 5:m.no] = b[:, 5:m.no] + torch.log(torch.tensor(0.6 / (m.nc - 0.99))) if cf is None else torch.log(cf / cf.sum())  # cls
